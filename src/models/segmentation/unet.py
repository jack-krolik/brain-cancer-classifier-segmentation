...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        # Encoder Block
        self.enc_conv_block_1 = UNetConvBlock(
            in_channels=3, out_channels1=64, out_channels2=64
        )
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.enc_conv_block_2 = UNetConvBlock(
            in_channels=64, out_channels1=128, out_channels2=128
        )
        self.enc_conv_block_3 = UNetConvBlock(
            in_channels=128, out_channels1=256, out_channels2=256
        )
        self.enc_conv_block_4 = UNetConvBlock(
            in_channels=256, out_channels1=512, out_channels2=512
        )
        self.enc_conv_block_5 = UNetConvBlock(
            in_channels=512, out_channels1=1024, out_channels2=1024
        )
        ...
        self.dec_conv_block_1 = UNetConvBlock(
            in_channels=1024, out_channels1=512, out_channels2=512
        )
        self.upconv_2 = nn.ConvTranspose2d(
            in_channels=512, out_channels=256, kernel_size=2, stride=2
        )
        self.dec_conv_block_2 = UNetConvBlock(
            in_channels=512, out_channels1=256, out_channels2=256
        )
        self.upconv_3 = nn.ConvTranspose2d(
            in_channels=256, out_channels=128, kernel_size=2, stride=2
        )
        self.dec_conv_block_3 = UNetConvBlock(
            in_channels=256, out_channels1=128, out_channels2=128
        )
        self.upconv_4 = nn.ConvTranspose2d(
            in_channels=128, out_channels=64, kernel_size=2, stride=2
        )
        self.dec_conv_block_4 = UNetConvBlock(
            in_channels=128, out_channels1=64, out_channels2=64
        )

        # Output Layer
        # TODO - double check the output layer
        self.output = nn.Conv2d(
            in_channels=64, out_channels=1, kernel_size=1, padding="valid"
        )  # TODO: double check on the padding
    ...
    @validate_model_input(
        {
            "shape": (
                ...
                320,
                320,
            ),  # TODO: might need to check self.in_channels instead of assuming 3
            "dims": [3, 4],
            "dtype": torch.float32,
    ...
    def forward(self, x):
        ...
        enc_2 = self.enc_conv_block_2(
            down_sample_1
        )  # Input shape is 64x160x160 -> 128x160x160
        down_sample_2 = self.max_pool(enc_2)  # Input shape is 128x160x160 -> 128x80x80
        enc_3 = self.enc_conv_block_3(
            down_sample_2
        )  # Input shape is 128x80x80 -> 256x80x80
        down_sample_3 = self.max_pool(enc_3)  # Input shape is 256x80x80 -> 256x40x40
        enc_4 = self.enc_conv_block_4(
            down_sample_3
        )  # Input shape is 256x40x40 -> 512x40x40
        down_sample_4 = self.max_pool(enc_4)  # Input shape is 512x40x40 -> 512x20x20
        encoding = self.enc_conv_block_5(
            down_sample_4
        )  # Input shape is 512x20x20 -> 1024x20x20

        # Decoder Block
        up_sample_1 = self.upconv_1(encoding)  # Input shape is 1024x20x20 -> 512x40x40
        skip_1 = self._skip_connection(
            up_sample_1, enc_4
        )  # Concatenate (512x40x40, 512x40x40) -> 1024x40x40
        ...
        skip_2 = self._skip_connection(
            up_sample_2, enc_3
        )  # Concatenate (256x80x80, 256x80x80) -> 512x80x80
        ...
        skip_3 = self._skip_connection(
            up_sample_3, enc_2
        )  # Concatenate (128x160x160, 128x160x160) -> 256x160x160
        dec_3 = self.dec_conv_block_3(
            skip_3
        )  # Input shape is 256x160x160 -> 128x160x160
        up_sample_4 = self.upconv_4(dec_3)  # Input shape is 128x160x160 -> 64x320x320
        skip_4 = self._skip_connection(
            up_sample_4, enc_1
        )  # Concatenate (64x320x320, 64x320x320) -> 128x320x320
        dec_4 = self.dec_conv_block_4(
            skip_4
        )  # Input shape is 128x320x320 -> 64x320x320
    ...
    def _skip_connection(self, decode_component, encode_component):
        ...
        if decoder_shape != encoder_shape:
            ...
            encode_component = encode_component[
                :, :, crop : crop + decoder_shape[0], crop : crop + decoder_shape[1]
            ]
    ...
    def _init_weights(self):
        ...
        def init_weights(m):
            if type(m) == nn.Conv2d or type(m) == nn.ConvTranspose2d:
                nn.init.normal_(
                    m.weight, std=m.in_channels * m.kernel_size[0] * m.kernel_size[1]
                )
